...
public class PartnerNewCleaningBooking {
...
@Given("^Header parameters are given for New Cleaning Booking api call$")
	...
	public void GetPartnerNewCleaningBooking()
...
public void getPartnerNewCleaningBookingcall()
{
	  response = request.
	   when().
	   get("v1.0/read/partner/getBookingLists?bookingType=NEW_CLEANING_BOOKINGS");
}

@Step
@Then("^all New Cleaning Booking get request should be success with status code 200$")

public void returnAllNewCleaningBooking()
{
	response.then().assertThat().statusCode(200).and()
	.contentType(ContentType.JSON).and()
	.body("success",equalTo(true)).and()
	.body("message",equalTo("Operation Completed Successfully. 42 result(s) returned")).and()
